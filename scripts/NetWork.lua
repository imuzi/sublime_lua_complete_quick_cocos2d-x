...
function NetWork.connect(hostt,portt)
    ...
    c:settimeout(0.1)
    local t = socket.gettime()
    local r, e = c:connect(hostt, portt)
    CCLuaLog("--------testSocket!--"..tostring(r).."+++"..tostring(e).."---------------------------")
    -- end
    if e then
        CCLuaLog("ife");
        else
...
--sl sleep
function NetWork.send(str,tm,sl)
    reconnect()
    local  err, total
   ...
   -- str = string.rep("a", 2*len)
    ...
    CCLuaLog("total"..tostring(total).."")
    CCLuaLog("err"..tostring(err).."")
    CCLuaLog("partial"..tostring(partial).."")
    CCLuaLog("elapsed"..tostring(elapsed).."")
   -- check_timeout(tm, sl, elapsed, err, "send", "total",
                --  total == 2)

end

function NetWork.receive(len, tm, sl)
    reconnect()
...
function test(tm, sl, elapsed, err, opp, mode, alldone)
    if tm < sl then
        if opp == "send" then
